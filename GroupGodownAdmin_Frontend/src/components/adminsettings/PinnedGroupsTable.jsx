...
async function fetchData() {
  ...
  try {
    ...
    let response = await fetch(
      `${baseUri}odata/Groups?$expand=Reports,Application,Category&$filter=Pin eq true`,
      {
        method: "GET",
        headers: headersList,
      }
  ...
  const unpinGroup = async () => {
    try{
      const response = await axios.put(
        `${baseUri}groups/Pin?id=${groupIdTounpin}`,
        {id : groupIdTounpin},
        {
          headers: {
                  ...
                  <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700 text-center">
                    ...
                    <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                      ...
                      .map((pinnedGroup,index) => {
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-black">
                          <a className="text-black" href={pinnedGroup.groupLink} target="_blank" rel="noopener noreferrer">{pinnedGroup.groupLink}</a>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-black">
                          {pinnedGroup.Category.catName}
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-black">
                          {pinnedGroup.country}
                        </td>
                                ...
                                >
                                  ...
                                  <DialogContent>
                                    <DialogContentText id="alert-dialog-description">
                                      Do you really want to unpin this group?
                                    </DialogContentText>
                                  </DialogContent>
                                  <DialogActions>
